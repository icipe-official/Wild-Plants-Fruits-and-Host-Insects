...
33. The 'insect_regions' table represent a many-to-many relationship between insects and regions, where one insect was reared in plants
from multiple regions, and one region can have multiple insects, and it has two columns:
...
36 The plants_matk table stores genetic data related to the matK gene in plants. The matK gene is a commonly used marker for plant species 
   identification and classification. The table establishes a one to many relationship between plant species and the matK barcode. This implies 
   that one plant species can have more than one barcode assigned to it. thi is often due to biological replicates or samples from different regions
   of the world.  FOREIGN KEY (plant_id) REFERENCES plants(id): Establishes a relationship with the 'plants' table,ensuring that a valid plant
   entry must exist for each matK entry. The table has four columns described below;
      "matk_id": Unique identifier for each matK genetic data entry. This is a primary key and it ensures the uniqueness of the matK genetic data entries.
      "plant_id" : Foreign key referencing the 'id' column in the 'plants' table,linking the genetic data to a specific plant entry
      "nucleotide" The genetic sequence data of the matK gene for a particular plant.
      "country":  A varying character column which stores information about the country where the plant sample was collected.
      "genebank_accession": A varying caharacter column that sores information about the accession number or identifier in a gene bank database
   

37. insects_coi  table stores genetic data related to the COI (Cytochrome c oxidase subunit I) gene in insects.The COI gene is a widely used marker
 for insect species identification and classification. PRIMARY KEY (coi_id) ensures the uniqueness of the COI genetic data entrieswhile the 
    FOREIGN KEY (insect_id) REFERENCES insects(id): Establishes a relationship with the 'insects' table,
    The table has four columns desccribed below 
   coi_id (INT): Unique identifier for each COI genetic data entry. The colunm ensures the uniqueness of the COI genetic data entries.
   insect_id (INT): Foreign key referencing the 'id' column in the 'insects' table,linking the genetic data to a specific insect entry.
   country (VARCHAR): The country where the insect sample was collected.
   genebank_accession (VARCHAR): Accession number or identifier in a gene bank database where the genetic data is stored.
    nucleotide (VARCHAR): The genetic sequence data of the COI gene for a particular insect.
                                              ensuring that a valid insect entry must exist for each COI entry.
*/
...
CREATE TABLE fruit_shapes(
       ...
       fruit_shape_description varchar
...
CREATE TABLE glossary(
    ...
    glossary_term varchar(50),
	glossary_description varchar(500),
	PRIMARY KEY (glossary_id) 
);

CREATE TABLE plants_matk (
  matk_id INT,
plant_id INT,
  nucleotide varchar,
  country varchar,
...
CREATE TABLE plant_coordinates (
  plant_id INT,
  latitude varchar,
  longitude varchar,
  PRIMARY KEY (plant_id, latitude,longitude),
  FOREIGN KEY (plant_id) REFERENCES plants (id) ON DELETE CASCADE ON UPDATE CASCADE
);
