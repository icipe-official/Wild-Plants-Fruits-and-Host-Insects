(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ? factory(exports, require("d3"), require("underscore"), require("lodash"))
    : typeof define === "function" && define.amd
    ? define(["exports", "d3", "underscore", "lodash"], factory)
    : ((global =
        typeof globalThis !== "undefined" ? globalThis : global || self),
      factory(
        (global.phylotree = global.phylotree || {}),
        global.d3,
        global._,
        global._$1
      ));
      ...
      trees = xml["nex:nexml"].trees[0].tree.map(function (nexml_tree) {
        ...
        function parseNexml(node, index) {
          if (node.edges) {
            ...
            node.children = ___namespace.values(
              ___namespace.pick(node_hash, targets)
            );
    ...
    if (!respect) {
      this.nodes.each((d) => {
        if (!isLeafNode(d)) {
          ...
          d.data.traits[this.selection_attribute_name] =
  ...
  function newickParser(nwk_str, options = {}) {
    let show_bootstrap = options.show_bootstrap || true;
    ...
    function finishNodeDefinition() {
      ...
      if ("children" in this_node) {
        if (show_bootstrap) {
          let bootstrap_value = current_node_name.match(/\[.*\]/);
          if (bootstrap_value) {
            this_node["bootstrap"] = parseFloat(
              bootstrap_value[0].slice(1, -1)
            );
          }
        }
  ...
  function parseAnnotations(buf) {
    ...
    // split on semicolon
    data = ___namespace.map(data_str.split(";"), (d) => {
      return d.trim();
    });

    // get dimensions
    let dimensions = ___namespace.filter(data, (d) => {
      return d.toUpperCase().startsWith("DIMENSION");
    });
    dimensions = dimensions[0].split(" ");
    dimensions = ___namespace.object(
      ___namespace.map(___namespace.rest(dimensions), (d) => {
        return d.split("=");
      })
    );

    // get formats
    let format = ___namespace.filter(data, (d) => {
      return d.toUpperCase().startsWith("FORMAT");
    });
    format = format[0].split(" ");
    format = ___namespace.object(
      ___namespace.map(___namespace.rest(format), (d) => {
        return d.split("=");
      })
    );
    format.symbols = ___namespace.reject(
      format.symbols.split(""),
      (d) => d == '"'
    );

    // get character matrix
    let matrix = ___namespace.filter(data, (d) => {
      return d.toUpperCase().startsWith("MATRIX");
    });
    matrix = matrix[0].split("\n");
    matrix = ___namespace.object(
      ___namespace.map(___namespace.rest(matrix), (d) => {
        return ___namespace.compact(d.split(" "));
      })
    );
  ...
  function loadAnnotations(tree, label, annotations) {
    ...
    ___namespace.each(tree.getTips(), (d) => {
  ...
  function loadTree(buf) {
    ...
    trees = ___namespace.filter(trees, (d) => {
  ...
  function getTipLengths() {
    ...
    let toExport = ___namespace.map(tips, (d) => {
  ...
  function getBranchLengths() {
    let bl = this.branch_length;
    return ___namespace.map(this.nodes.descendants(), (node) => {
      return bl(node);
    });
  }
  ...
  function normalize(attr) {
    ...
    let branch_lengths = ___namespace.map(
  ...
  function reroot(node, fraction) {
    ...
    if (node.parent) {
      ...
      let apportioned_bl =
        node.data.__mapped_bl === undefined
          ? undefined
          : node.data.__mapped_bl * fraction;
    ...
    if (!___namespace.isUndefined(this.display)) {
      ...
      d3__namespace
        .select(rendered_tree.container)
        .node()
        .appendChild(rendered_tree.show());
      ...
      if (this.options["draw-size-bubbles"]) {
        ...
        if (this.shown_font_size >= 5) {
          labels = labels.attr("dx", (d) => {
            // eslint-disable-line
            return (
              (d.text_align == "end" ? -1 : 1) * this.shown_font_size * 0.33
            );
          });
        }
      }
  ...
  // modified tree visualization to show boostrap values (modified !this.internalNames from this.internalnames)
  ...
  //modified version to displat boostraps
    ...
    if (transitions) {
      ...
      collapsed_clades
        ...
        .attr("d", function (d) {
          return d.collapsed_clade
            ? d.collapsed_clade
            : (d.collapsed_clade = spline(d.collapsed));
        });
    }
  }
      ...
      if (!isNaN(parseInt(plantInsectId)) && isNaN(parseInt(Insectgenus))) {
        menu_object
          ...
          .on("click", (d) => {
            ...
            const detailsPageURL = `/plants/${plantInsectId}`;
    ...
    if (this.options["restricted-selectable"].length) {
      ...
      if (
        ___namespace.contains(
          ___namespace.keys(predefined_selecters),
          node_selecter
        )
      ) {
  ...
  class TreeRender {
    ...
    initialize_svg(svg_element) {
      ...
      if (this.svg !== svg_element) {
        d3__namespace.select(svg_element).select("svg").remove();
    ...
    update(transitions) {
      ...
      if (this.draw_scale_bar) {
        ...
        scale_bar
          ...
          .style(
            "font-size",
            this.ensure_size_is_in_px(this.scale_bar_font_size)
          )
          .merge(scale_bar)
          .attr("transform", (d) => {
            return this.d3PhylotreeSvgTranslate([
      ...
      let drawn_nodes = enclosure
        ...
        .data(this.phylotree.nodes.descendants().filter(nodeVisible), (d) => {
          return d.id || (d.id = ++node_id);
        });
      ...
      drawn_nodes = drawn_nodes
        ...
        .attr("transform", (d) => {
          if (
            !___namespace.isUndefined(d.screen_x) &&
            !___namespace.isUndefined(d.screen_y)
          ) {
      ...
      if (this.options["zoom"]) {
        let zoom = d3__namespace
          ...
          .on("zoom", (event) => {
            ...
            d3__namespace
              .select("." + css_classes["tree-scale-bar"])
              .attr("transform", (d) => {
                let toTransform = event.transform;
                toTransform.y -= 10;
                return toTransform;
    ...
    placenodes() {
      ...
      this.max_depth = d3__namespace.max(
        this.phylotree.nodes.descendants(),
        (n) => {
          return n.depth;
        }
      );
      ...
      if (this.options["top-bottom-spacing"] == "fixed-step") {
        ...
        this.scales[0] =
          (this.size[0] - this.pad_height()) / this._extents[0][1];
        at_least_one_dimension_fixed = true;
      }
    ...
    node_circle_size(attr, attr2) {
      ...
      this.options["node_circle_size"] = constant(
        attr === undefined ? 3 : attr
      );
      return this;
    }
  ...
  function mrca() {
    ...
    this.traverse_and_compute(function (node) {
      if (!node.children) {
        ...
        node.mrca = ___namespace.union(
          ...node.descendants().map((child) => child.mrca)
        );
  ...
  function computePairwiseDistances(tree) {
    ...
    tree.traverse_and_compute(function (n) {
      ...
      if (n.parent && ___namespace.isUndefined(n.cot_computed_length)) {
        throw (
          "Non-null branch lengths are required for this operation: missing branch length at node " +
          n.data.name
        );
      }
    ...
    tree.traverse_and_compute(function (n) {
      if (n.parent) {
        ___namespace.each(
          n.cot_path_to_leaves_below,
          function (length_so_far, leaf_index) {
      ...
      for (var this_node = 0; this_node < a_node.children.length; this_node++) {
        for (
          ...
          if (this_node != other_node) {
            ___namespace.each(
              a_node.children[other_node].cot_path_to_leaves_below,
    ...
    tree.traverse_and_compute(function (n) {
      if (n.parent) {
        ...
        ___namespace.each(
          n.parent.cot_path_to_leaves_above,
          function (length_so_far, leaf_index) {
  ...
  function sackin(tree) {
    ...
    let depths = ___namespace.map(tips, (d) => {
      return d.depth;
    });

    return ___namespace.reduce(
      depths,
      function (memo, num) {
        return memo + num;
      },
  ...
  function computeRootToTipOtherRoot(
    ...
    node.data.rtta =
      node.data.rootToTip - shared_distance + distance_to_new_root;
  ...
  function fitRootToTip(tree) {
    ...
    tree.traverse_and_compute(function (node) {
      if (
        ...
        !___namespace.isNull(node.data.decimal_date_value)
      ) {
        linear_data.push([
    ...
    if (n <= 2) {
      ...
      if (internalNodes.length) {
        let highestNum = ___namespace$1.max(
          ___namespace$1.map(
            internalNodes,
            (label) => ___namespace$1.split(label, "InternalNode")[1]
          )
        );
      ...
      ___namespace$1.each(___namespace$1.range(1, n - 1), (l) =>
        distanceMatrixArr[l].unshift(newRow[l])
      );
