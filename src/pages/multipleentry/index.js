...
import QuerySelect from "components/multipleentry/multipleentrySelectMUI"
import { Container } from "@mui/material"
export default function GroupCheckboxMin(props){
    return(
        <Container> 
          <QuerySelect
          fruit_color_data={props.fruit_color_data} 
          plant_type_data={props.plant_type_data}
          fruit_type_data={props.fruit_type_data}
          fruit_shape_data={props.fruit_shape_data}
          fruit_size_data={props.fruit_size_data}
          leaf_type_data={props.fruit_type_data}
          plants_data={props.plants_data}
          leaf_margin_data={props.leaf_margin_data}
          leaf_arrangement_data={props.leaf_arrangement_data}
          latex_data={props.latex_data}
          spines_data={props.spines_data}
          />
       </Container>)}
export async function getStaticProps(){
  // console.log('context.params');
    const size = await fetch('http://localhost:3000/api/multipleEntry/checkboxFruitsize')
    const fruit_size_data= await size.json()
    //  console.log('data')

    //fruit color data
     const type = await fetch('http://localhost:3000/api/multipleEntry/checkboxColor')
     const fruit_color_data= await type.json()
     console.log(fruit_size_data)

    // plant_type data
    const plant = await fetch('http://localhost:3000/api/multipleEntry/plantType')
    const plant_type_data= await plant.json()
    console.log(plant_type_data)


     // fruit_type data
     const fruit_type = await fetch('http://localhost:3000/api/multipleEntry/fruitType')
 ...
 // fruit_shape data
 const fruit_shape = await fetch('http://localhost:3000/api/multipleEntry/fruitShape')
 ...
 console.log(fruit_shape_data)
  ...
  const plants= await fetch('http://localhost:3000/api/multipleEntry/leafType')
...
const margins= await fetch('http://localhost:3000/api/multipleEntry/leafMargin')
...
const arrangements= await fetch('http://localhost:3000/api/multipleEntry/leafArrangement')
...
const spines= await fetch('http://localhost:3000/api/multipleEntry/thornsSpines')
