...
export default function FruitCheckbox(Query_params) {
  ...
  const fruit_size_array = Array.from(Query_params.params.fruit_size_data).map(
    (specie) =>
  ...
  const fruit_color_array = Array.from(
    Query_params.params.fruit_color_data
  ).map(
    (specie) =>
  ...
  const handleData = () => {
    ...
    fetch(`api/plants/prisma_api/multipleEntry/multipletrial?filters=${i}`)
      .then((res) => res.json())
      .then(
