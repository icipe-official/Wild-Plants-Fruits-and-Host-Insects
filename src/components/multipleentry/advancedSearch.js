...
export default function QuerySelectAdvancedSearch(props) {
  //handle cases if there is error in url
  // Inside your component
  const router = useRouter();
  const errorMessage = router.query.error ? router.query.error : null;
  //set keys for plant,fruits and leaf details
  ...
  if (data) {
    ...
    return (
      <Container sx={{ marginTop: isSmallScreen ? 10 : 10 }}>
        {isSmallScreen ? (
          <>
            {/* {errorMessage ? <Box>{errorMessage}</Box> : null} */}
            {errorMessage ? <PopupMessage errorMessage={errorMessage} /> : null}
            <SubmitClearquery
