...
export default function UKWFValues({ ukwf_area_data }) {
  ...
  return ukwf_area_data.map((ukwf, index) => (
    <Box sx={{ display: "flex", alignItems: "center" }}>
      <Box sx={{ width: "100%" }}>UKWF region:</Box>
      <Box sx={{ width: "100%" }}>
        <DynamicValueCardUKWFRegion
          key={index}
          keyName="K sector"
          value={`${ukwf.plants_ukwf_areas.map(
            (area) => area.ukwf_areas.ukwf_area
          )} `}
        />
      </Box>
    </Box>
