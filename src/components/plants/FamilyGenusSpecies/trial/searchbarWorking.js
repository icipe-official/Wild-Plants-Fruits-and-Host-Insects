...
console.log(familyId)
                        ...
                        .then(
                            (result) => {
                                ...
                                return specie.genus_id == e.target.value &&
                                        specie.genus.family_id==familyId});    /// mapping ids of family with that stored in familyid
                                setLoaded(true);
    ...
    const OnSpeciesChange = (e) => {
      const value=e.target.value;
        setSpeciesId(value);
        window.location.href='/plants/prisma_api/' + value.split(' ').slice(1, ).join(' ') // split with space, splice to get species name
